...
async def assemble_video(tool_context: ToolContext) -> str:
    ...
    try:
        ...
        for i, (temp_image, temp_audio) in enumerate(zip(temp_image_paths, temp_audio_paths)):
            ...
            )) if result.returncode == 0 else scenes[i].get("duration", 4)

            # 실제 음성 길이에 맞춰 영상 길이 조정
            total_frames = int(30 * actual_audio_duration)
            filter_parts.append(
            ...
            # 음성 길이를 정확히 맞춤
            filter_parts.append(
                f"[{i*2+1}:a]atrim=0:{actual_audio_duration},asetpts=PTS-STARTPTS[a{i}]")
        ...
        print(
            f"✅ Video assembly complete! Created {total_duration}s YouTube Short")
