...
@input_guardrail
async def off_topic_guardrail(wrapper: RunContextWrapper[UserAccountContext], agent: Agent[UserAccountContext], input: str):
...
triage_agent = Agent(
    ...
    # Agent를 Tool로 사용하는 방법
    # tools=[
    #     account_agent.as_tool(tool_name="Account Management Agent", description="Handle account access, security, and profile management issues."),
    #     technical_agent.as_tool(tool_name="Technical Support Agent", description="Solve technical issues with our products and services."),
    #     order_agent.as_tool(tool_name="Order Management Agent", description="Handle order status, shipping, returns, and delivery issues."),
    #     billing_agent.as_tool(tool_name="Billing Support Agent", description="Resolve billing, payment, and subscription issues."),
    # ],
    handoffs=[
